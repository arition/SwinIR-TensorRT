...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim: int, window_size: Tuple[int, int], num_heads: int, qkv_bias=True, qk_scale: Optional[float] = None, attn_drop=0., proj_drop=0.):
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, x_size: Tuple[int, int]):
        ...
        if self.input_resolution == x_size:
            ...
            attn_windows = self.attn(x_windows, mask=self.calculate_mask(x_size).cuda())
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        B, L, C = x.shape
        assert L == H * W, "input feature has wrong size"
        assert modulo(H, 2) == 0 and modulo(W, 2) == 0, f"x size ({H}*{W}) are not even."
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim: int, input_resolution: Tuple[int, int], depth: int, num_heads: int, window_size: int,
                 mlp_ratio=4., qkv_bias=True, qk_scale: Optional[float] = None, drop=0., attn_drop=0.,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 shift_size=0 if (modulo(i, 2) == 0) else window_size // 2,
                                 mlp_ratio=mlp_ratio,
...
class RSTB(nn.Module):
    ...
    def __init__(self, dim: int, input_resolution: Tuple[int, int], depth: int, num_heads: int, window_size: int,
                 mlp_ratio=4., qkv_bias=True, qk_scale: Optional[float] = None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, downsample: Optional[torch.nn.Module] = None, use_checkpoint=False,
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer: Optional[torch.nn.Module] = None):
        super().__init__()
...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
                 ...
                 window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale: Optional[float] = None,
    ...
    def check_image_size(self, x: torch.Tensor):
        _, _, h, w = x.size()
        mod_pad_h = modulo((self.window_size - modulo(h, self.window_size)), self.window_size)
        mod_pad_w = modulo((self.window_size - modulo(w, self.window_size)), self.window_size)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.upsampler == 'nearest+conv':
            ...
            x = self.conv_after_body(self.forward_features(x)) + x
            x = self.conv_before_upsample(x)
            x = self.lrelu(self.conv_up1(torch.nn.functional.interpolate(x, scale_factor=2.0, mode='nearest')))
            if self.upscale == 4:
                x = self.lrelu(self.conv_up2(torch.nn.functional.interpolate(x, scale_factor=2.0, mode='nearest')))
